...
| 替换模式（工具箱） | 转换结果**直接写回原文件**，不在相册生成新文件；源与产物数量不一致时按位对应（1→2 多出的产物写到源文件旁，2→1 删掉多出的伴生文件）；**必须先授予「所有文件访问」**，与「删除原图」互斥，首次开启弹一次风险与免责提示（可不再提示） |
...
| 转码方式三选一 | 不符合标准 MP4 的视频由用户决定怎么处理：重新编码（兼容性最好）／仅重封装容器（默认，只换容器不重压，快且无损）／完全不使用转码器（只接受 MP4 与可零拷贝转换的 MOV）；不改变编码的路径统一复核产物编码，绝不产出能识别却放不出来的坏动态照片 |
| 转码进度 | 合成转码时在对应列表项内展示「已处理帧 / 总帧数」进度条与预计剩余时间，起止平滑展开收缩 |
| 高刷新率 | 完整版启动时自动请求设备支持的最高刷新率档位（60 / 75 / 90 / 120 / 144 / 165 / 180 / 185 / 240Hz 等按机型上限），动画跑满屏幕帧率上限 |
...
- 就地替换：在「工具箱 → 替换模式」里开启后，转换结果会**直接写回原文件**
  （需先授予「所有文件访问」；首次开启会弹一次风险提示，勾选「不再提示」后不再打扰）
...
| 转换产物 | 相册目录 `Pictures/Z-LivePhoto-Converter/`；开启「按源文件夹层级输出」时为其下的原相册名子目录；开启**替换模式**时不产生新文件，结果直接覆盖源文件 |
